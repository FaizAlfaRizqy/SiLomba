        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div x-data="{ tab: '{{ $totalLamaranMasuk > 0 ? 'lamaran-masuk' : ($timSayaKetuai->count() > 0 ? 'tim-kelola' : ($lamaranPending->count() > 0 ? 'pending' : 'aktif')) }}' }">
                ...
                <div class="flex border-b border-[#E2E8F0] overflow-x-auto no-scrollbar mb-8 gap-2">
                    @if($timSayaKetuai->count() > 0)
                    <button @click="tab = 'lamaran-masuk'" :class="tab === 'lamaran-masuk' ? 'border-b-2 border-[#4F7EF7] text-[#4F7EF7] font-bold bg-white' : 'text-[#64748B] hover:text-[#051F20]'" class="px-6 py-4 text-sm transition-all whitespace-nowrap flex items-center rounded-t-xl">
                        📨 Lamaran Masuk
                    ...
                    <button @click="tab = 'tim-kelola'" :class="tab === 'tim-kelola' ? 'border-b-2 border-[#4F7EF7] text-[#4F7EF7] font-bold bg-white' : 'text-[#64748B] hover:text-[#051F20]'" class="px-6 py-4 text-sm transition-all whitespace-nowrap flex items-center rounded-t-xl">
                        👑 Tim Saya Kelola
                        <span class="ml-2 bg-[#4F7EF7] text-white text-[10px] font-black px-2 py-0.5 rounded-full">{{ $timSayaKetuai->count() }}</span>
                    </button>
                    @endif

                    <button @click="tab = 'pending'" :class="tab === 'pending' ? 'border-b-2 border-[#4F7EF7] text-[#4F7EF7] font-bold bg-white' : 'text-[#64748B] hover:text-[#051F20]'" class="px-6 py-4 text-sm transition-all whitespace-nowrap flex items-center rounded-t-xl">
                        ⏳ Menunggu Review
                        @if($lamaranPending->count() > 0)
                    ...
                    <button @click="tab = 'aktif'" :class="tab === 'aktif' ? 'border-b-2 border-[#4F7EF7] text-[#4F7EF7] font-bold bg-white' : 'text-[#64748B] hover:text-[#051F20]'" class="px-6 py-4 text-sm transition-all whitespace-nowrap flex items-center rounded-t-xl">
                        ✅ Tim yang Diikuti
                    ...
                    <button @click="tab = 'ditolak'" :class="tab === 'ditolak' ? 'border-b-2 border-[#4F7EF7] text-[#4F7EF7] font-bold bg-white' : 'text-[#64748B] hover:text-[#051F20]'" class="px-6 py-4 text-sm transition-all whitespace-nowrap rounded-t-xl">
                        ❌ Ditolak
                    </button>

                    <button @click="tab = 'riwayat'" :class="tab === 'riwayat' ? 'border-b-2 border-[#4F7EF7] text-[#4F7EF7] font-bold bg-white' : 'text-[#64748B] hover:text-[#051F20]'" class="px-6 py-4 text-sm transition-all whitespace-nowrap rounded-t-xl">
                        🕐 Riwayat
                    </button>
                </div>
                ...
                <div x-show="tab === 'lamaran-masuk'" class="space-y-8">
                    @if($totalLamaranMasuk > 0)
                        <div class="flex items-center justify-between mb-4">
                            <p class="text-sm text-[#64748B]">Total <span class="font-bold text-[#051F20]">{{ $totalLamaranMasuk }}</span> lamaran menunggu keputusanmu</p>
                        </div>
                                        ...
                                        <div class="bg-white border border-[#E2E8F0] rounded-[2rem] p-6 hover:shadow-xl transition-all group">
                                            <div class="flex flex-col md:flex-row justify-between gap-6">
                                                ...
                                                <div class="flex gap-5 flex-1">
                                                    ...
                                                    <div class="space-y-1">
                                                        <h4 class="font-black text-[#051F20] text-lg">{{ $lamaran->pelamar->name }}</h4>
                                            ...
                                            <div class="mt-6" x-data="{ buka: false }">
                                                ...
                                                <div x-show="buka" x-transition class="mt-3 bg-[#EFF6FF] border-l-4 border-[#4F7EF7] rounded-xl p-4 text-sm text-[#051F20] leading-relaxed italic shadow-inner">
                                                    ...
                                                    <div x-data="{ modal: false, alasan: '' }">
                                                        ...
                                                        <div x-show="modal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4" x-cloak>
                                                            <div class="bg-white rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl scale-up" @click.away="modal = false">
                                                                ...
                                                                <h4 class="text-2xl font-black text-[#051F20] text-center mb-2">Tolak Lamaran?</h4>
                                                                <p class="text-sm text-[#64748B] text-center mb-8">Tolak lamaran dari <span class="font-bold text-[#051F20]">{{ $lamaran->pelamar->name }}</span>? Berikan alasan agar pelamar bisa belajar lebih baik.</p>
                                                    ...
                                                    <div x-data="{ modal: false }">
                                                        ...
                                                        <div x-show="modal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4" x-cloak>
                                                            <div class="bg-white rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl scale-up" @click.away="modal = false">
                                                                ...
                                                                <h4 class="text-2xl font-black text-[#051F20] text-center mb-2">Terima Anggota Baru?</h4>
                                                                <p class="text-sm text-[#64748B] text-center mb-8">Terima <span class="font-bold text-[#051F20]">{{ $lamaran->pelamar->name }}</span> sebagai anggota tim? Mahasiswa ini akan langsung masuk ke grup chat tim.</p>
                    ...
                    @else
                        <div class="bg-white rounded-[3rem] p-20 text-center shadow-sm border border-[#E2E8F0]">
                            ...
                            <h3 class="text-2xl font-black text-[#051F20]">Belum ada lamaran masuk</h3>
                            ...
                            <div class="p-8">
                                <div class="flex flex-col md:flex-row justify-between gap-6 mb-8">
                                    <div>
                                        ...
                                        <h3 class="text-3xl font-black text-[#051F20]">{{ $tim->nama_tim }}</h3>
                                    </div>
                                    <div class="flex gap-2">
                                        ...
                                        <a href="{{ route('mahasiswa.my-teams.show', $tim->id) }}" class="px-6 py-3 bg-[#F8FAFC] border border-[#E2E8F0] text-[#051F20] text-xs font-black uppercase tracking-widest rounded-2xl hover:bg-gray-50 transition">
                                            Detail
                                        </a>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <!-- Anggota Terdaftar -->
                                    <div>
                                        <h4 class="text-xs font-black text-[#051F20] mb-4 uppercase tracking-widest flex items-center gap-2">
                                        ...
                                        <div class="space-y-3">
                                            @foreach($tim->anggota as $member)
                                                <div class="flex items-center gap-4 p-4 rounded-2xl bg-[#F8FAFC] border border-[#E2E8F0]">
                                                    ...
                                                    <div class="flex-1">
                                                        <h5 class="text-sm font-bold text-[#051F20]">{{ $member->user->name }}</h5>
                                            ...
                                            @foreach($tim->slots as $slot)
                                                ...
                                                <div class="p-4 rounded-2xl border border-[#E2E8F0] {{ $slot->status == 'buka' ? 'bg-white' : 'bg-gray-50 opacity-60' }}">
                                                    <div class="flex justify-between items-start mb-2">
                                                        <h5 class="text-sm font-black text-[#051F20]">{{ $slot->posisi }}</h5>
                ...
                <div x-show="tab === 'pending'" class="space-y-4">
                        ...
                        <div class="bg-white border border-amber-200 rounded-[2rem] p-6 shadow-sm hover:shadow-lg transition-all">
                            <div class="flex flex-col md:flex-row justify-between items-start gap-4">
                                <div class="flex-1">
                                    ...
                                    <h3 class="text-2xl font-black text-[#051F20]">{{ $lamaran->slot->tim->nama_tim }}</h3>
                                ...
                                <div class="text-right">
                                    ...
                                    <p class="text-sm font-black text-[#051F20]">{{ $lamaran->created_at->format('d M Y, H:i') }}</p>
                                </div>
                            </div>
                            ...
                            <div class="bg-[#F8FAFC] rounded-[1.5rem] p-5 mt-5 grid grid-cols-1 md:grid-cols-2 gap-4 border border-[#E2E8F0]">
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm font-bold text-[#051F20]">{{ $lamaran->slot->tim->lomba->nama }}</p>
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm font-black {{ $lamaran->slot->tim->lomba->deadline->diffInDays(now()) <= 7 ? 'text-red-500' : 'text-[#051F20]' }}">
                            ...
                            <div class="flex flex-col sm:flex-row justify-between items-center mt-8 pt-8 border-t border-[#E2E8F0] gap-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs font-black text-[#051F20]">{{ $lamaran->slot->tim->ketua->name }}</p>
                                    </div>
                                </div>
                                <div class="flex gap-3 w-full sm:w-auto">
                                    <div x-data="{ konfirmasi: false }">
                                        ...
                                        <div x-show="konfirmasi" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" x-cloak>
                                            <div class="bg-white rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl scale-up text-center" @click.away="konfirmasi = false">
                                                ...
                                                <h4 class="text-2xl font-black text-[#051F20] mb-2">Batal Melamar?</h4>
                                                <p class="text-sm text-[#64748B] mb-8">Apakah kamu yakin ingin menarik kembali lamaran untuk tim <span class="font-bold text-[#051F20]">{{ $lamaran->slot->tim->nama_tim }}</span>?</p>
                                                <div class="flex gap-3">
                    ...
                    @empty
                        <div class="bg-white rounded-[3rem] p-20 text-center shadow-sm border border-[#E2E8F0]">
                            ...
                            <h3 class="text-2xl font-black text-[#051F20]">Belum ada lamaran pending</h3>
                ...
                <div x-show="tab === 'aktif'" class="space-y-6">
                            ...
                            <div class="bg-[#F8FAFC] px-8 py-6 border-b border-[#E2E8F0] group-hover:bg-[#EFF6FF] transition">
                                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                    <div>
                                        ...
                                        <h3 class="text-2xl font-black text-[#051F20]">{{ $anggota->tim->nama_tim }}</h3>
                            ...
                            <div class="p-8">
                                <div class="grid grid-cols-3 gap-6 bg-[#F8FAFC] rounded-2xl p-6 mb-8 border border-[#E2E8F0]">
                                    <div class="text-center">
                                        <p class="text-xl font-black text-[#051F20]">{{ $anggota->tim->lomba->tingkat }}</p>
                                    ...
                                    <div class="text-center border-x border-[#E2E8F0]">
                                        <p class="text-xl font-black text-[#051F20]">{{ $anggota->tim->lomba->deadline->format('d M') }}</p>
                                ...
                                <div>
                                    <h4 class="text-[10px] font-black text-[#051F20] mb-4 uppercase tracking-widest flex items-center gap-2">Rekan Tim</h4>
                                        ...
                                        @foreach($anggota->tim->anggota as $member)
                                            <div class="flex items-center gap-3 p-3 rounded-2xl bg-white border border-[#E2E8F0] hover:border-[#4F7EF7]/30 transition group/member">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <h5 class="text-xs font-bold text-[#051F20] truncate">{{ $member->user->name }}</h5>
                            ...
                            <div class="bg-white px-8 py-6 border-t border-[#E2E8F0] flex flex-col sm:flex-row justify-between items-center gap-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs font-black text-[#051F20]">{{ $anggota->tim->ketua->name }}</p>
                                    </div>
                                </div>
                                <div class="flex gap-3 w-full sm:w-auto">
                                    ...
                                    <a href="{{ route('mahasiswa.my-teams.show', $anggota->tim->id) }}" class="flex-1 sm:flex-none flex items-center justify-center px-8 py-3 bg-[#F8FAFC] border border-[#E2E8F0] text-[#051F20] text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-gray-50 transition">
                                        Detail
                                    </a>
                                </div>
                            </div>
                    ...
                    @empty
                        <div class="bg-white rounded-[3rem] p-20 text-center shadow-sm border border-[#E2E8F0]">
                            ...
                            <h3 class="text-2xl font-black text-[#051F20]">Belum bergabung di tim manapun</h3>
                        ...
                        <div class="bg-white border border-red-100 rounded-[2rem] p-8 shadow-sm hover:shadow-md transition">
                            <div class="flex flex-col md:flex-row items-start gap-6">
                                ...
                                <div class="flex-1">
                                    ...
                                    <h3 class="text-2xl font-black text-[#051F20] mb-1">{{ $lamaran->slot->tim->nama_tim }}</h3>
                ...
                <div x-show="tab === 'riwayat'" class="space-y-4">
                    ...
                    @forelse($riwayat as $lamaran)
                        <div class="bg-white border border-[#E2E8F0] rounded-[2rem] p-6 opacity-60 grayscale hover:grayscale-0 transition">
                            ...
                            <h3 class="text-xl font-black text-[#051F20]">{{ $lamaran->slot->tim->nama_tim }}</h3>
