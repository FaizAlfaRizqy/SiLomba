            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="divide-y divide-gray-50">
                            @foreach($tim->anggota as $anggota)
                                <div class="p-6 flex items-center justify-between">
                                    <div class="flex items-center space-x-4">
                                        <div class="w-12 h-12 rounded-2xl bg-[#0B2B26]/10 flex items-center justify-center text-[#0B2B26]">
                            ...
                            @foreach($tim->slots as $slot)
                                <div class="p-4 bg-gray-50 rounded-2xl border border-gray-100 flex items-center justify-between">
                                    ...
                                    <div class="text-right">
                                        <span class="block text-xs font-bold text-[#0B2B26]">{{ $slot->jumlah_slot }} Slot</span>
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden" 
                         ...
                         x-init="init()">
                        <div class="p-6 border-b border-gray-50 flex items-center justify-between bg-[#0B2B26] text-white">
                            <h3 class="font-bold">Diskusi Tim</h3>
                            <span class="text-[10px] px-2 py-0.5 bg-[#235347] rounded-full font-bold uppercase tracking-widest">Real-time</span>
                        </div>
                        <div class="h-96 overflow-y-auto p-6 space-y-4 flex flex-col" id="chat-container">
                            <template x-for="msg in messages" :key="msg.id">
                                <div :class="msg.id_pengirim == {{ Auth::id() }} ? 'self-end text-right' : 'self-start'">
                                    ...
                                    <div :class="msg.id_pengirim == {{ Auth::id() }} ? 'bg-[#0B2B26] text-white rounded-2xl rounded-tr-none' : 'bg-gray-100 text-gray-800 rounded-2xl rounded-tl-none'" 
                        ...
                        <div class="p-4 bg-gray-50 border-t border-gray-100">
                            <form @submit.prevent="sendMessage()" class="flex space-x-2">
                                <input type="text" x-model="newMessage" placeholder="Ketik pesan..." class="flex-1 rounded-xl border-gray-300 text-sm focus:ring-[#0B2B26] focus:border-[#0B2B26]">
                                <button type="submit" class="p-2 bg-[#0B2B26] text-white rounded-xl hover:bg-[#0B2B26]/90 transition">
                            ...
                            @foreach($tim->slots as $slot)
                                @foreach($slot->lamarans->where('status', 'pending') as $lamaran)
                                    ...
                                    <div class="space-y-4 pb-6 border-b border-gray-50 last:border-0 last:pb-0">
                                        ...
                                        <a href="{{ route('mahasiswa.portfolio', $lamaran->pelamar->mahasiswa->nim) }}" class="block text-center text-[10px] font-bold text-[#0B2B26] hover:underline">Lihat Profil Lengkap &rarr;</a>
                                    </div>
                                @endforeach
                            @endforeach
